...
public class ComplytGtRatesEndpointsIT extends TestContainersInitializerIT implements ComplytGtRatesEndpointsITTemplate {
    ...
    @Test
    @Override
    public void findByAddress_FindsGtAddressWithCountryAndRegionMisspell_ReturnsComplytGtRates() {
        WEB_TEST_CLIENT
                .get()
                .uri(uriBuilder -> uriBuilder
                        .path(TestUtilities.COMPLYT_GT_RATES_BASE_URL)
                        .queryParam("country", "Canada")
                        .queryParam("region", "Quebek")
                        .build())
                .headers(headers -> {
                    headers.setBearerAuth(TOKEN);
                    headers.setContentType(MediaType.APPLICATION_JSON);
    ...
    @Test
    @Override
    public void findByAddress_FindsGtAddressWithCountryDoesNotExist_Return404NotFound() {
        WEB_TEST_CLIENT
                .get()
                .uri(uriBuilder -> uriBuilder
                        .path(TestUtilities.COMPLYT_GT_RATES_BASE_URL)
                        .queryParam("country", "not_exist")
                        .build())
                .headers(headers -> {
                    headers.setBearerAuth(TOKEN);
                    headers.setContentType(MediaType.APPLICATION_JSON);
    ...
    @Test
    @Override
    public void findByAddress_FindsGtAddressWithCountryAndRegionNotExist_ReturnsComplytGtRates() {
        WEB_TEST_CLIENT
                .get()
                .uri(uriBuilder -> uriBuilder
                        .path(TestUtilities.COMPLYT_GT_RATES_BASE_URL)
                        .queryParam("country", "Canada")
                        .queryParam("region", "not_exist")
                        .build())
                .headers(headers -> {
                    headers.setBearerAuth(TOKEN);
                    headers.setContentType(MediaType.APPLICATION_JSON);
                ...
                .expectBody()
                .jsonPath("$.gtAddress.country").isEqualTo("Canada")
                .jsonPath("$.gtAddress.region").doesNotExist()
                .jsonPath("$.gtRates.taxRate").isEqualTo("0.05")
                .jsonPath("$.gtRates.countryRate").isEqualTo("0.05");
    }
