...
public class WebhookEntityCreator<T extends ComplytIdProperty> {
    ...
    public Mono<WebhookEntityWrapper<T>> create(Class<T> webhookClass, T object) {
        LocalDateTime timestamp = LocalDateTime.now();
        UUID id = UUID.randomUUID();
        WebhookEntityWrapper<T> webhookEntityWrapper = new WebhookEntityWrapper<>(id, timestamp, webhookClass, object);
