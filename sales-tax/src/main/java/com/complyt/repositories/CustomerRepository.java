    ...
    public Flux<Customer> findAll(int page, int size, Map<String, String> filterMap, String sortOrder, String sortBy) {
        ...
        return TenantResolver.resolve()
                .flatMapMany(tenantId -> {
                    ...
                    return ContextLogger.observeCtx("Searching for Customers by criteria " + criteria.getCriteriaObject() + ", sorting by " + sortByProperty + ", ordered " + sortDirection + ", with page " + page + " and size " + size, tenantId, log::info)
                            .thenMany(reactiveMongoTemplate.find(query, Customer.class));
                });
    }
