...
public class EconomicNexusByYearFromSeptemberToSeptemberIT extends TestContainersInitializerIT implements EconomicNexusOnlyTaxableItemsITTemplate {
    ...
    private final ShippingAddressDto referenceAddress = new ShippingAddressDto("West Haven", "US", null, "CT", "300 Boston Post Rd", "", "06516", false, new MatchedAddressDataDto(new MandatoryAddressDto("West Haven", "US", null, "CT", "300 Boston Post Rd", "", "06516", false), null));
    ...
    public void upsertTransaction_NewAndDoesntPassedEconomicNexus_Returns201() {
        ...
        TransactionDto givenTransaction = ITUtilities.stubTransactionDto(externalId, customerId,
                        ITUtilities.stubItemDto().withUnitPrice(new BigDecimal(4)).withQuantity(new BigDecimal(15000)).withTotalPrice(new BigDecimal(60000)))
    ...
    public void upsertTransaction_NewAndNotTaxableItem_Returns201() {
        ...
        TransactionDto givenTransaction = ITUtilities.stubTransactionDto(externalId, customerId,
                        ITUtilities.stubItemDto().withQuantity(new BigDecimal(6)).withUnitPrice(new BigDecimal(10000)).withTotalPrice(new BigDecimal(60000)).withTaxCode("C?S1")) // A tax code which in CT is not taxable
                .withShippingAddress(referenceAddress)
                .withExternalTimestamps(new TimestampsDto(referenceDate.toString(), LocalDateTime.now().toString()));
    ...
    public void getSalesTaxTracking_CheckEconomicNexusNotPassed_Returns200() {
        webTestClient
                .mutate().responseTimeout(Duration.ofMinutes(2)).build().get()
                .uri(uriBuilder -> uriBuilder
